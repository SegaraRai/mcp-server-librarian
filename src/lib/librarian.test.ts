...
vi.mock('./load.js', () => {
  ...
  return {
    ...
    filterDocuments: vi.fn().mockImplementation((cache, directory = '/', tags = []) => {
      if (tags.length > 0) {
    ...
    searchDocuments: vi.fn().mockImplementation((cache, query, directory = '/', tags = [], includeContents = false) => {
    ...
    getTagsInDirectory: vi.fn().mockImplementation((cache, directory = '/', includeFilepaths = false) => {
      const tags = [
        { tag: 'tag1', count: 1 },
        { tag: 'tag2', count: 2 },
        { tag: 'tag3', count: 1 }
...
describe('Librarian', () => {
  ...
  describe('listDocuments', () => {
    it('should call loadAllDocuments and filterDocuments', async () => {
      const result = await librarian.listDocuments({ directory: '/', tags: [] });
    ...
    it('should filter documents by tags', async () => {
      const result = await librarian.listDocuments({ directory: '/', tags: ['tag1'] });
