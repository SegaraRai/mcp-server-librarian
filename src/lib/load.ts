 ...
 */
function getInheritedTags(docsRoot: string, filepath: string, documentMap: Map<string, RawDocument>): string[] {
 ...
 */
export async function loadAllDocuments(docsRoot: string): Promise<DocumentCache> {
  ...
  for (const rawDoc of rawDocuments) {
    try {
      const enrichedDoc = enrichDocumentWithTags(docsRoot, rawDoc, rawDocumentMap);
...
export function filterDocuments(
  ...
  const dirFiltered = cache.documents.filter(doc => 
    normalizedDir === "" || doc.filepath.startsWith(dirPrefix)
  );
...
export function searchDocuments(
  ...
  const results = filtered.filter(doc => 
    doc.contents && regex.test(doc.contents)
  );
